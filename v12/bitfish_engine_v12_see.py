...
from v12.bitfish_board_v12_see import (
    Board,
    Move,
    WHITE,
    BP,
...
# Used only for capture ordering and pruning obviously bad captures.
# ------------------------------------------------------------------
BAD_CAPTURE_SEE_MARGIN = -150
...
class Engine:
    def __init__(self) -> None:
        ...
        self.see_cache: dict[tuple[int, tuple[int, int, int | None, bool, bool]], int] = {}
    ...
    def quiescence(self, board: Board, alpha: int, beta: int, ply: int) -> int:
        ...
        for move in moves:
            if move.promotion is None and self.see_capture(board, move) < BAD_CAPTURE_SEE_MARGIN:
                continue
    ...
    def piece_value_on_square(self, board: Board, square: int) -> int:
        piece = board.piece_at(square)
        return 0 if piece is None else PIECE_VALUE[piece]

    def least_valuable_legal_capture_to(
        self,
        board: Board,
        target_square: int,
    ) -> Move | None:
    ...
    def see_capture_uncached(self, board: Board, move: Move) -> int:
        captured_piece = board.piece_at(move.to_sq)

        if move.is_en_passant:
            captured_value = PIECE_VALUE[BP]
        elif captured_piece is not None:
            captured_value = PIECE_VALUE[captured_piece]
        else:
            captured_value = 0

        if captured_value == 0 and move.promotion is None:
            return 0
        ...
        if move.promotion is not None:
            promotion_bonus = PIECE_VALUE[move.promotion] - PIECE_VALUE[attacker]

        gains = [captured_value + promotion_bonus]

        test_board = board.copy()
        side = test_board.side_to_move
        undo = test_board.make_move(move)

        if test_board.in_check(side):
            test_board.unmake_move(undo)
            return -INF // 2

        target_square = move.to_sq

        while True:
            reply = self.least_valuable_legal_capture_to(test_board, target_square)
            if reply is None:
                break

            piece_on_target_value = self.piece_value_on_square(test_board, target_square)
            reply_attacker = test_board.piece_at(reply.from_sq)

            if reply_attacker is None:
                break

            reply_promotion_bonus = 0
            if reply.promotion is not None:
                reply_promotion_bonus = (
                    PIECE_VALUE[reply.promotion] - PIECE_VALUE[reply_attacker]
                )

            gains.append(piece_on_target_value + reply_promotion_bonus - gains[-1])
            test_board.make_move(reply)
    ...
    def see_capture(self, board: Board, move: Move) -> int:
        key = (board.hash_key, move_key(move))
        cached = self.see_cache.get(key)

        if cached is not None:
            return cached

        score = self.see_capture_uncached(board, move)
    ...
    def capture_score(self, board: Board, move: Move) -> int:
        if move.promotion is not None:
            ...
            return 30_000 + 10 * see + PIECE_VALUE[move.promotion]

        if move.is_en_passant:
            see = self.see_capture(board, move)
            return 20_000 + 10 * see
        ...
        # SEE is the main ordering term. MVV-LVA remains as a tie-breaker.
        return (
            20_000
            + 10 * see
            + 10 * PIECE_VALUE[victim]
