...
from v9.bitfish_board_v9 import (
    Board,
    Move,
    WHITE,
    PIECE_VALUE,
...
class SearchResult:
    ...
    quiet_eval_calls: int
    quiet_eval_time: float
    movegen_calls: int
    movegen_time: float
    futility_prunes: int
    delta_prunes: int
...
class Engine:
    def __init__(self) -> None:
        ...
        self.full_eval_time = 0.0
        self.quiet_eval_calls = 0
        self.quiet_eval_time = 0.0
        self.movegen_calls = 0
        self.movegen_time = 0.0
    ...
    def search_best_move(
        ...
        for depth in range(1, max_depth + 1):
            ...
            print(
                ...
                f"nodes {self.nodes} main {self.main_nodes} q {self.q_nodes} "
                f"evals {self.eval_calls} evalms {int(self.eval_time * 1000)} "
                f"fullevals {self.full_eval_calls} fullms {int(self.full_eval_time * 1000)} "
                f"qevals {self.quiet_eval_calls} qevalms {int(self.quiet_eval_time * 1000)} "
                f"gen {self.movegen_calls} genms {int(self.movegen_time * 1000)} "
                f"tthits {self.tt_hits} "
                f"fut {self.futility_prunes} delta {self.delta_prunes} "
                f"nullcut {self.null_prunes} lmr {self.lmr_reductions} "
                f"research {self.lmr_researches} beta {self.beta_cutoffs} "
                f"checks {self.time_checks} "
        ...
        return SearchResult(
            ...
            full_eval_time=self.full_eval_time,
            quiet_eval_calls=self.quiet_eval_calls,
            quiet_eval_time=self.quiet_eval_time,
            movegen_calls=self.movegen_calls,
            movegen_time=self.movegen_time,
            futility_prunes=self.futility_prunes,
            delta_prunes=self.delta_prunes,
    ...
    def evaluate_position(self, board: Board) -> int:
        ...
        self.full_eval_time += elapsed

        return score

    def evaluate_quiet_position(self, board: Board) -> int:
        start = time.perf_counter()
        score = board.evaluate_quiet()
        elapsed = time.perf_counter() - start

        self.eval_calls += 1
        self.eval_time += elapsed
        self.quiet_eval_calls += 1
        self.quiet_eval_time += elapsed
    ...
    def generate_moves(self, board: Board) -> list[Move]:
        ...
        self.movegen_calls += 1
        self.movegen_time += time.perf_counter() - start
        return moves
...
def print_result(result: SearchResult) -> None:
    ...
    print(f"main nodes: {result.main_nodes}")
    print(f"q nodes: {result.q_nodes}")
    print(f"eval calls: {result.eval_calls}")
    print(f"eval time: {result.eval_time:.3f}s")
    print(f"full eval calls: {result.full_eval_calls}")
    print(f"full eval time: {result.full_eval_time:.3f}s")
    print(f"quiet eval calls: {result.quiet_eval_calls}")
    print(f"quiet eval time: {result.quiet_eval_time:.3f}s")
    print(f"movegen calls: {result.movegen_calls}")
    print(f"movegen time: {result.movegen_time:.3f}s")
    print(f"futility prunes: {result.futility_prunes}")
    print(f"delta prunes: {result.delta_prunes}")
    print(f"null prunes: {result.null_prunes}")
    print(f"lmr reductions: {result.lmr_reductions}")
    print(f"lmr re-searches: {result.lmr_researches}")
    print(f"beta cutoffs: {result.beta_cutoffs}")
    print(f"time checks: {result.time_checks}")
