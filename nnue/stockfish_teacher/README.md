...
- `train_external.py`: imports the existing Sgurr model, loss and exporter;
  saves optimiser/scheduler/RNG state for resume.
...
Sgurr-X's authorised sequence is `x_series.json`: 112M/HL384, then
826M/HL384 with eight factorised king buckets, scale 361 and score-only targets.
