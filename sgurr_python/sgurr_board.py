...
OPENING_MAX_FULLMOVE = 3
OPENING_MIN_NON_PAWN_MATERIAL = 5200

OPENING_CENTRE_PAWN_BONUS = 22
OPENING_KINGSIDE_KNIGHT_BONUS = 18
OPENING_QUEENSIDE_KNIGHT_BONUS = 8
...
for _f in range(8):
    _adj = 0
    if _f > 0:
        _adj |= FILE_MASKS[_f - 1]
    if _f < 7:
        _adj |= FILE_MASKS[_f + 1]
    ADJACENT_FILE_MASKS.append(_adj)
...
class Board:
    ...
    def non_pawn_material_total(self) -> int:
        total = 0

        for piece in (WN, WB, WR, WQ, BN, BB, BR, BQ):
            total += PIECE_VALUE[piece] * self.bitboards[piece].bit_count()

        return total

    def opening_phase_active(self) -> bool:
        if self.fullmove_number > OPENING_MAX_FULLMOVE:
            return False

        return self.non_pawn_material_total() >= OPENING_MIN_NON_PAWN_MATERIAL

    def evaluate_opening_principles_for_colour(self, colour: int) -> int:
        if colour == WHITE:
            pawns = self.bitboards[WP]
            knights = self.bitboards[WN]
            bishops = self.bitboards[WB]
            king = self.bitboards[WK]

            centre_pawn_squares = (square_index("d4"), square_index("e4"))
            kingside_knight_square = square_index("f3")
            queenside_knight_square = square_index("c3")
            bishop_home_squares = (square_index("c1"), square_index("f1"))
            castled_king_squares = (square_index("g1"), square_index("c1"))
        else:
            pawns = self.bitboards[BP]
            knights = self.bitboards[BN]
            bishops = self.bitboards[BB]
            king = self.bitboards[BK]

            centre_pawn_squares = (square_index("d5"), square_index("e5"))
            kingside_knight_square = square_index("f6")
            queenside_knight_square = square_index("c6")
            bishop_home_squares = (square_index("c8"), square_index("f8"))
            castled_king_squares = (square_index("g8"), square_index("c8"))

        score = 0

        for sq in centre_pawn_squares:
            if pawns & bit(sq):
        ...
        for sq in bishop_home_squares:
            if not (bishops & bit(sq)):
                score += OPENING_BISHOP_DEVELOPMENT_BONUS

        for sq in castled_king_squares:
            if king & bit(sq):
    ...
    def evaluate_opening_principles(self) -> int:
        if not self.opening_phase_active():
            return 0

        return (
            self.evaluate_opening_principles_for_colour(WHITE)
    ...
    def evaluate_pawn_structure_for_colour(self, colour: int) -> int:
        ...
        if colour == WHITE:
            ...
            enemy_pawns = self.bitboards[BP]
            forward = 8
            support_rank_range = lambda pawn_rank: range(0, pawn_rank + 1)
            passed_bonus_index = lambda pawn_rank: pawn_rank
            enemy_attackers_of_front = lambda front_sq: self.pawn_attacks_from(front_sq, WHITE)
        else:
            own_pawns = self.bitboards[BP]
            enemy_pawns = self.bitboards[WP]
            forward = -8
            support_rank_range = lambda pawn_rank: range(pawn_rank, 8)
            passed_bonus_index = lambda pawn_rank: 7 - pawn_rank
            enemy_attackers_of_front = lambda front_sq: self.pawn_attacks_from(front_sq, BLACK)
        ...
        while bb:
            ...
            for ahead_rank in ranks_ahead:
                for adjacent_file in (f - 1, f, f + 1):
            ...
            for behind_rank in behind_ranks:
            ...
            if on_board(front_sq):
                front_attacked_by_enemy_pawn = bool(
                    enemy_pawns & enemy_attackers_of_front(front_sq)
                )

                support_mask = 0

                for adjacent_file in (f - 1, f + 1):
                    if not (0 <= adjacent_file < 8):
                        continue

                    for support_rank in support_rank_range(r):
                        support_mask |= bit(support_rank * 8 + adjacent_file)

                can_be_supported = bool(own_pawns & support_mask)

                if front_attacked_by_enemy_pawn and not can_be_supported:
    ...
    def evaluate_pawn_structure(self) -> int:
        return (
            self.evaluate_pawn_structure_for_colour(WHITE)
    ...
    def evaluate_king_safety_for_colour(self, colour: int) -> int:
        """
        Returns a score (always positive = king is safe) for `colour`.
        Penalties for:
          - Enemy pieces attacking squares adjacent to the king
          - Open/semi-open files next to the king (no friendly pawn shield)
        """
        king_sq = self.king_square(colour)
        if king_sq == -1:
            return 0
        ...
        for f in range(max(0, king_file - 1), min(8, king_file + 2)):
            ...
            if not pawns_on_file:
                ...
                while pawn_bb:
                    ...
                    if colour == WHITE and rank_of(psq) > rank_of(king_sq) + 2:
                        score -= KING_OPEN_FILE_PENALTY // 2
                    elif colour == BLACK and rank_of(psq) < rank_of(king_sq) - 2:
                        score -= KING_OPEN_FILE_PENALTY // 2
    ...
    def evaluate_king_safety(self) -> int:
        return (
            self.evaluate_king_safety_for_colour(WHITE)
    ...
    def evaluate_mobility_for_colour(self, colour: int) -> int:
        """
        Returns a mobility bonus for `colour` based on how many squares
        bishops, rooks, and queens can reach (excluding own-occupied squares).
        """
        occ = self.occupancy()
        own = self.occupancy(colour)
        score = 0

        if colour == WHITE:
            bishops = self.bitboards[WB]
            rooks = self.bitboards[WR]
            queens = self.bitboards[WQ]
        else:
            bishops = self.bitboards[BB]
            rooks = self.bitboards[BR]
            queens = self.bitboards[BQ]

        bb = bishops
        while bb:
            sq, bb = pop_lsb(bb)
            moves = self.attacks_from_slider(sq, BISHOP_DELTAS, occ) & ~own
            score += moves.bit_count() * BISHOP_MOBILITY_BONUS

        bb = rooks
        while bb:
            sq, bb = pop_lsb(bb)
            moves = self.attacks_from_slider(sq, ROOK_DELTAS, occ) & ~own
            score += moves.bit_count() * ROOK_MOBILITY_BONUS

        bb = queens
        while bb:
            sq, bb = pop_lsb(bb)
            moves = self.attacks_from_slider(sq, QUEEN_DELTAS, occ) & ~own
            score += moves.bit_count() * QUEEN_MOBILITY_BONUS

        return score

    def evaluate_mobility(self) -> int:
        return (
            self.evaluate_mobility_for_colour(WHITE)
    ...
    def evaluate(self) -> int:
        ...
        score += self.evaluate_opening_principles()
...
def test_evaluation_and_perft() -> None:
    ...
    board = Board()
    knight_move = Move(square_index("b1"), square_index("c3"))
    undo = board.make_move(knight_move)
    nc3_eval = board.evaluate()
    board.unmake_move(undo)

    board = Board()
    pawn_move = Move(square_index("e2"), square_index("e4"))
    board.make_move(pawn_move)
    e4_opening_eval = board.evaluate()

    assert e4_opening_eval < nc3_eval, (
        f"expected e2e4 to score better than b1c3 for White, "
        f"got e2e4={e4_opening_eval}, b1c3={nc3_eval}"
    )

    print(f"evaluation test passed, e2e4 score for black to move: {e4_eval}")
    print("opening principle test passed")
